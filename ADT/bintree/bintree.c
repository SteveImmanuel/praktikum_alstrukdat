...
{
    addrNode pt;
    pt=(addrNode)malloc(sizeof(Node));
    if (pt!=Nil){
        Akar(pt)=X;
        Left(pt)=Nil;
        Right(pt)=Nil;
...
void PrintTreeRec(BinTree P, int h, int level){
    if(!IsTreeEmpty(P)){
        int i;
        for(i=1;i<=h*(level-1);i++){
            printf(" ");
        }
        printf("%d\n",Akar(P));
        PrintTreeRec(Left(P),h,level+1);
        PrintTreeRec(Right(P),h,level+1);
    }     
}
...
{
    if(IsTreeOneElmt(*P)){ //kalau satu elemen pasti bernilai X
        if(Akar(*P)==X){
            addrNode temp=*P;
            DealokNode(temp);
            *P=Nil;
   ...
   Menghasilkan list kosong jika ada alokasi yang gagal. */
{
    if (IsTreeEmpty(P)){
        ...
        if(IsTreeOneElmt(P)){
            ...
            // List tempL=Nil,tempR=Nil;
            // if(IsUnerLeft(P)){
            //     return MakeListDaun(Left(P));
            // }else if(IsUnerRight(P)){
            //     return MakeListDaun(Right(P));
            // }else if(IsBiner(P)){
            //     tempL=MakeListDaun(Left(P));
            //     tempR=MakeListDaun(Right(P));
            //     if(tempL!=Nil && tempR!=Nil){
            //         return Concat(tempL,tempR);
            //     }else{
            //         return Nil;
            //     }
            // }    
   ...
   Menghasilkan list kosong jika ada alokasi yang gagal. */
{
    if(IsTreeEmpty(P)){
        ...
        if(N==1){
            ...
            // }else if(IsBiner(P)){
            //     tempL=MakeListLevel(Left(P),N-1);
            //     tempR=MakeListLevel(Right(P),N-1);
            //     if(tempL!=Nil && tempR!=Nil){
            //         return Concat(tempL,tempR);
            //     }else{
            //         return Nil;
